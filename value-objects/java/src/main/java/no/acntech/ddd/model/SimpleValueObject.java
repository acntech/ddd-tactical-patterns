...
public class SimpleValueObject<P extends Comparable<P>> implements PrimitiveValueObject<P> {
  ...
  /**
   * Protected constructor - subclasses <i>must</i> call this in order for the framework to work. The primitive value is validated
   * by first checking the range extremes (if any) and then calling the possibly overridden {@link #validate} method.
   */
